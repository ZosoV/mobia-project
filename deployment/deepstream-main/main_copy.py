...
def parse_args():
    # Defining parser
    parser_description = "RTSP Output Sample Application Help "
    input_help = "List of paths to input H264 elementry streams"
    ...
    if len(sys.argv) == 1:
        ...
        sys.exit(1)
    # Convert to attributes namespace and make them global variables
...
def main(args):
    ...
    if len(args) < 2:
        ...
        sys.exit(1)
    # Initialize and assign stream FPS values
    ...
    # adding link to the queues, tracker and sgie1
    queue2.link(tracker)
    tracker.link(queue3)
    queue3.link(sgie1)
    sgie1.link(queue4)
